...
function applyScriptBindingGraph(opts) {
  ...
  // Pass 1c: React setState ObjectLiteral → ObjLiteralProp transfers
  ...
  try {
    for (const call of sf.getDescendantsOfKind(SyntaxKind.CallExpression)) {
      ...
      if (!arg0 || arg0.getKindName() !== 'ObjectLiteralExpression') continue;
      const stateName =
  ...
  // Pass 4: Ant Design Table — dataSource={alias} + columns[].dataIndex → item props
  try {
