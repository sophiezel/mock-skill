...
function resolveRelativePath(urlPath, maps) {
  ...
  const list = maps || [];
  if (!list.length) {
    return [{ host: '_default', path: urlPath.split('?')[0], confidence: 'low', prefixKey: null }];
  }
  ...
  /** @type {Map<string, { host: string, path: string, confidence: string, prefixKey: string }>} */
