...
function createBindingGraph(shape, responsePaths = []) {
  ...
  function applyMember(aliasKey, memberParts) {
    ...
    if (!b) return;
    if (b.kind === 'item') {
      ensureItemProp(b.path, memberParts, b.under || null);
      return;
    }
    if (b.kind === 'root') {
   ...
   * @param {string[]} [under] - nested array field under parent item (e.g. ['itemList'])
   */
  function bindItemAlias(itemAlias, arrayPath, under = null) {
    if (under?.length) {
      markNestedItemArray(arrayPath, under);
      bindAlias(itemAlias, {
        kind: 'item',
        path: [...arrayPath],
        under: [...under],
  ...
  /**
   * Mark arrayPath.item.under… as array (nested collection under list item).
   * @param {string[]} arrayPath
   * @param {string[]} under
   */
  function markNestedItemArray(arrayPath, under) {
    if (!under?.length) return;
    markArray(arrayPath);
    const arrNode = arrayPath.length ? getNodeAtPath(arrayPath) : shape;
    let cur = arrNode.item || { type: 'object', props: {} };
    arrNode.item = cur;
    if (!cur.props) cur.props = {};
    for (let i = 0; i < under.length; i++) {
      const p = under[i];
      if (!cur.props[p]) cur.props[p] = { type: 'object', props: {} };
      if (i === under.length - 1) {
        cur.props[p].type = 'array';
        if (!cur.props[p].item) cur.props[p].item = { type: 'object', props: {} };
        if (!cur.props[p].item.props) cur.props[p].item.props = {};
      } else {
        if (!cur.props[p].props) cur.props[p] = { type: 'object', props: {}, enums: cur.props[p].enums };
        cur = cur.props[p];
      }
    }
  }

  function ensureItemProp(arrayPath, memberParts, under = null) {
    if (!memberParts.length) return;
    if (under?.length) {
      markNestedItemArray(arrayPath, under);
      const arrNode = arrayPath.length ? getNodeAtPath(arrayPath) : shape;
      let cur = arrNode.item;
      for (const u of under) {
        cur = cur.props[u].item;
      }
      if (!cur.props) cur.props = {};
