...
const FIDELITY_LEVELS = {
  L0: {
    ...
    upgradeHint: 'Run `mock-skill capture-merge` after browsing the flow, or `import-openapi` to supply shape.',
  },
  L1: {
    label: 'schema/usage shape + placeholder',
    description: 'Response shape inferred from usage (or OpenAPI); values are placeholders.',
    upgradeHint: 'Run `mock-skill session` + `capture-merge` to replace placeholders with real values.',
  },
  L2: {
