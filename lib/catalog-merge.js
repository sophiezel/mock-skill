 ...
 * Truth: services/<upstream>/contracts; also reads project mirror for legacy.
 * Dedupes by stubId || id.
...
function loadContractsForCatalog(catalogKey) {
  ...
  addDir(path.join(projectDataDir(catalogKey), 'contracts'));
  ...
  if (legacyProject && legacyProject !== catalogKey) {
    addDir(path.join(projectDataDir(legacyProject), 'contracts'));
  }
  // Pure service mount with no index: expandMountKey already listed it
  if (!services.length && !legacyProject) {
    addDir(path.join(serviceDataDir(catalogKey), 'contracts'));
  }
