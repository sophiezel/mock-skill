...
function renderHandler(contract) {
  ...
  return `/** Auto-generated by mock-skill — ${contract.id}${gaps ? ` gaps=${gaps}` : ''}${layer} */
...
function pruneOrphanArtifacts(projectSlug, { keepHandlerKeys, keepContractKeys }) {
  ...
  function pruneHandlerFile(full, stopDir, stubIdCandidates) {
    ...
    if (content.includes('mock-skill:manual')) return;
    fs.unlinkSync(full);
    prunedHandlers++;
    ...
    if (isEmptyContractOnly(contract)) {
      ...
      if (fs.existsSync(handlerFile)) {
        try {
          ...
          if (!prev.includes('mock-skill:manual')) fs.unlinkSync(handlerFile);
        } catch {
          /* ignore */
        }
      }
