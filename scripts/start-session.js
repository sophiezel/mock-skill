...
const { loadSession, saveRuntimeState } = require('../lib/session-config');
...
async function startSession(opts = {}) {
  ...
  let cfg = loadSession(projectSlug);
  if (opts.mockPort) cfg.mock.port = Number(opts.mockPort);
  if (opts.proxyPort) cfg.proxy.port = Number(opts.proxyPort);
  ...
  if (opts.startUrl) cfg.browser.startUrl = opts.startUrl;
  if (opts.autoLaunch === false) cfg.browser.autoLaunch = false;
  ...
  const proxyServerArg = `${proxyHost}:${proxyPort}`;
  ...
  if (cfg.proxy.enabled && cfg.browser.autoLaunch && chrome && fs.existsSync(chrome)) {
    ...
    else args.push(`http://${proxyHost}:${proxyPort}/`);
...
module.exports = { startSession, findChrome };
